...
type OpenApiBuilderMethods = {
  [K in keyof OpenApiBuilderProperties]-?: (
    val: OpenApiBuilderProperties[K],
  ) => OpenApiBuilder;
};
...
export class OpenApiBuilder implements OpenApiBuilderMethods {
  ...
  private createPropertyHandler(
    prop: keyof OpenApiBuilderMethods,
  ): (val) => this {
  ...
  public zod(): typeof z {
    ...
    const enrichZodSchemaWithOpenApi =
      (fn) =>
      (...args) => {
